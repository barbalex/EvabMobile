...
window.em.initiiereFeldEdit_2 = function() {
	...
	if (window.Feld.Hierarchiestufe === "Art") {
		window.em.ArtGruppeAufbauenFeldEdit(window.Feld.ArtGruppe);
	}
...
window.em.initiierehBeobEdit = function() {
	...
	if (window.hArt && (!localStorage.Von || localStorage.Von !== "hArtEdit")) {
		window.em.initiierehBeobEdit_2(window.hArt);
	} else {
		$db = $.couch.db("evab");
...
function handleHOrtEditPageinit() {
	...
	$("#OrtEditHeader").on("click", "[name='OeffneOrtListeOrtEdit']", function(event) {
		event.preventDefault();
		handleHOrtEditOeffneOrtListeClick();
	});

	$("#OrtEditHeader").on("click", "#OeffneRaumOrtEdit", function(event) {
		event.preventDefault();
		handleHOrtEditOeffneRaumClick();
	});
	...
	$("#OrtEditHeader").on("click", "#OeffneProjektOrtEdit", function(event) {
		event.preventDefault();
		handleHOrtEditOeffneProjektClick();
	});
	...
	$('#OrtEditFooter').on('click', '#KarteOeffnenOrtEdit', function(event) {
		event.preventDefault();
		handleHOrtEditKarteOeffnenClick();
	});
...
function handleHOrtEditLoescheOrtClick(that) {
...
function handleHOrtListePageinit() {
	...
	$("#hOrtListePageFooter").on('click', '#OeffneKarteOrtListe', function(event) {
		event.preventDefault();
		handleHOrtListeOeffneKarteClick();
	});
...
function nächsteVorigeArt_2(NächsteOderVorige) {
	...
	for (i in window.hBeobListe.rows) {
		...
		AnzArt = window.hBeobListe.rows.length -1;  // vorsicht: Objekte zählen Elemente ab 1, Arrays ab 0!
...
function löscheHBeob_2() {
	...
	$db.removeDoc(window.hArt, {
		success: function (data) {
			...
			if (window.hBeobListe) {
				for (i in window.hBeobListe.rows) {
					if (window.hBeobListe.rows[i].doc._id === data.id) {
